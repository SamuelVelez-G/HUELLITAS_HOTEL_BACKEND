...
public class Detalle_reservaService {
    ...
    public Detalle_reservaResponseDTO crear(Detalle_reservaRequestDTO dto) {
        Reserva reserva = reservaRepo.findById(dto.getReservaId())
                .orElseThrow(() ->
                        new RuntimeException("Reserva no encontrada: " + dto.getReservaId()));
        Mascota mascota = mascotaRepo.findById(dto.getMascotaId())
                .orElseThrow(() ->
                        new RuntimeException("Mascota no encontrada: " + dto.getMascotaId()));
        Servicio servicio = servicioRepo.findById(dto.getServicioId())
                .orElseThrow(() ->
                        new RuntimeException("Servicio no encontrado: " + dto.getServicioId()));
        ...
        reserva.setTotal(
                reserva.getTotal().add(BigDecimal.valueOf(subtotal))
        );
        // Agregar el detalle a la reserva
        reserva.getDetalles().add(saved);
    ...
    public void eliminar(Long id) {
        ...
        // Restar subtotal del total de la reserva
        reserva.setTotal(
                reserva.getTotal().subtract(BigDecimal.valueOf(d.getSubtotal()))
        );
        // Eliminar el detalle de la lista
        reserva.getDetalles()
                .removeIf(det -> det.getId().equals(id));
        reservaRepo.save(reserva);
